...
class Diffusion:
    ...
    def noise_schedule(self) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Define the linear noise schedule for the diffusion process.
    ...
    def add_noise(
        ...
        alpha_bar_t = (
            self.alpha_bar[t].view(-1, 1).to(self.device)
        )  # Shape: (batch_size, 1)
    ...
    def train(self, epochs: int) -> None:
        ...
        for epoch in range(1, epochs + 1):
            ...
            for batch_idx, batch in enumerate(self.data_loader, 1):
                ...
                t = torch.randint(
                    0, self.n_steps, (batch.size(0),), device=self.device
                ).long()
                ...
                if batch_idx % 100 == 0:
                    logging.info(
                        f"Epoch [{epoch}/{epochs}], Batch [{batch_idx}], Loss: {loss.item():.4f}"
                    )
        ...
        with torch.no_grad():
            for _ in range(num_samples):
                ...
                for step in reversed(range(self.n_steps)):
                    t_step = torch.full(
                        (x.size(0),), step, device=self.device, dtype=torch.long
                    )
    ...
    def save_generated_data(
        self, coordinates: np.ndarray, energies: np.ndarray
    ) -> None:
        """
        ...
        try:
            with open(snapshot_path, "w") as fw:
                for i in range(len(coordinates)):
                    ...
                    fw.write(
                        f"{energies[i]:.6f} runpoint {int(self.runpoint[i % len(self.runpoint)])}\n"
                    )
                    for a in range(len(self.atoms)):
                        fw.write(
                            f"{self.atoms[a]} {coordinates[i][a][0]:.6f} {coordinates[i][a][1]:.6f} {coordinates[i][a][2]:.6f}\n"
                        )
